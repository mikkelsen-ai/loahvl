...
export default function Footer() {
  return (
    <footer style={{ background: '#07202f' }} className="py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <p className="font-serif font-bold text-white text-xl tracking-wide mb-1">LØA</p>
            <p className="text-gray-500 text-sm">{SITE_CONFIG.fullName} · HVL</p>
            <p className="text-gray-500 text-sm">{SITE_CONFIG.address}</p>
            <a
              href={`mailto:${SITE_CONFIG.email}`}
              className="text-gray-500 text-sm hover:text-accent transition-colors duration-200"
            >
              {SITE_CONFIG.email}
            </a>
          </div>
          <div className="flex gap-3">
            <a
              ...
              className="w-9 h-9 bg-[#1877f2] rounded-lg flex items-center justify-center text-white font-bold hover:opacity-90 transition-opacity"
            >
              f
            </a>
            <a
              ...
              className="w-9 h-9 rounded-lg flex items-center justify-center text-white hover:opacity-90 transition-opacity"
        ...
        <div className="border-t border-primary/60 pt-4">
          <p className="text-gray-600 text-xs">
            © {new Date().getFullYear()} LØA – {SITE_CONFIG.fullName} ved HVL
          </p>
        </div>
      </div>
    </footer>
